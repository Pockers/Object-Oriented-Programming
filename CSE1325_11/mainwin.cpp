...
void Mainwin::on_create_order_click() {
	...
	// Create Label that will be updated with the Order object
	Gtk::Label l_order;
	dialog->get_vbox()->pack_start(l_order, Gtk::PACK_SHRINK);
	// Create Combobox for products to be inserted into the order
	Gtk::ComboBoxText c_products;
	c_products.set_size_request(200);
	for (int i = 0; i < _store.number_of_products(); i++) {
		c_products.append(_store.product_to_string(i));
	}
	dialog->get_vbox()->pack_start(c_products, Gtk::PACK_SHRINK);
